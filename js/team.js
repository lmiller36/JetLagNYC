...
function initTeamPage() {
    ...
    // Inject sign-in modal into the notLoggedIn div
    const notLoggedInDiv = document.getElementById('notLoggedIn');
    if (notLoggedInDiv) {
        if (window.createSignInModal) {
            notLoggedInDiv.innerHTML = window.createSignInModal('Sign in to manage your team and view scores.');
        } else {
            // Fallback if createSignInModal isn't loaded yet
            notLoggedInDiv.innerHTML = `
                <div style="display: flex; justify-content: center; align-items: center; min-height: 400px; padding: 20px;">
                    ...
                    ">
                        ...
                        <p style="margin-bottom: 24px; color: var(--dark-color); line-height: 1.6;">Sign in to manage your team and view scores.</p>
                        <button
