// Embedded location-specific challenges data for file:// protocol compatibility
window.CHALLENGES_LOCATION = {
  "challenges": [
    {
      "id": "location-001",
      "title": "Take a picture with a sexy princess in Times Square",
      "description": "Take a picture with anyone in a costume in Times Square (bonus for hottest princess)",
      "category": "location-specific",
      "basePoints": 5,
      "bonusPoints": {"criteria": "Hottest princess", "points": 3},
      "locationRestriction": {"type": "neighborhood", "value": "times-square"},
      "requirements": ["Must be in Times Square", "Person must be in costume"]
    },
    {
      "id": "location-002",
      "title": "Get high on the High Line",
      "description": "Act like a stoner on the High Line (no need to actually get high, unless you want to). Hoodies and snacks encouraged",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "high-line"},
      "requirements": ["Must be on the High Line", "Video evidence", "Stay in character"]
    },
    {
      "id": "location-003",
      "title": "Whisper secrets at Grand Central echo wall",
      "description": "Whisper secrets at the Grand Central whispering gallery",
      "category": "location-specific",
      "basePoints": 7,
      "locationRestriction": {"type": "neighborhood", "value": "grand-central"},
      "requirements": ["Must be at the whispering gallery", "Video evidence"]
    },
    {
      "id": "location-004",
      "title": "Take tram to Roosevelt Island",
      "description": "Ride the Roosevelt Island Tramway",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "roosevelt-island"},
      "requirements": ["Must ride the tram", "Photo from inside tram"]
    },
    {
      "id": "location-005",
      "title": "Read the story of Dumbo in DUMBO",
      "description": "Read the story of Dumbo in DUMBO. The rest of the team has to sit down and listen like it's story time in kindergarten",
      "category": "location-specific",
      "basePoints": 11,
      "locationRestriction": {"type": "neighborhood", "value": "dumbo"},
      "requirements": ["Must be in DUMBO", "Team must sit and listen", "Video required"]
    },
    {
      "id": "location-006",
      "title": "Take a timeout at Time Out Market",
      "description": "Take a timeout at Time Out Market. Take fun group picture on the roof",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "dumbo"},
      "requirements": ["Must be at Time Out Market", "Group photo on roof"]
    },
    {
      "id": "location-007",
      "title": "Hate America at the Statue of Liberty",
      "description": "Give at least 10 reasons America sucks with the Statue of Liberty in view on camera",
      "category": "location-specific",
      "basePoints": 8,
      "locationRestriction": {"type": "neighborhood", "value": "statue-of-liberty"},
      "requirements": ["Statue of Liberty must be visible", "At least 10 reasons", "Video required"]
    },
    {
      "id": "location-008",
      "title": "Outshine the Swedish - build something new at IKEA Red Hook",
      "description": "Build something new at IKEA Red Hook",
      "category": "location-specific",
      "basePoints": 15,
      "locationRestriction": {"type": "neighborhood", "value": "red-hook"},
      "requirements": ["Must be at IKEA Red Hook", "Must build something", "Photo of creation"]
    },
    {
      "id": "location-009",
      "title": "Fake a proposal with Empire State Building view",
      "description": "Fake a proposal with a view of the Empire State Building (bonus points for most convincing)",
      "category": "location-specific",
      "basePoints": 10,
      "bonusPoints": {"criteria": "Most convincing proposal", "points": 5},
      "locationRestriction": {"type": "neighborhood", "value": "manhattan"},
      "requirements": ["Empire State Building must be visible", "Video required", "Must look convincing"]
    },
    {
      "id": "location-010",
      "title": "Prospect in Prospect Park",
      "description": "Pick up 10 pieces of trash as a team in Prospect Park (best pile of gold wins). You can do this in any park for 6 points, Prospect Park gets 10",
      "category": "location-specific",
      "basePoints": 10,
      "bonusPoints": {"criteria": "Best pile of trash/gold", "points": 5},
      "locationRestriction": {"type": "neighborhood", "value": "prospect-park"},
      "requirements": ["Pick up 10 pieces of trash", "Photo of collected trash", "Any park: 6 points, Prospect Park: 10 points"]
    },
    {
      "id": "location-011",
      "title": "Scale a wall on Wall Street",
      "description": "Scale/climb a wall on Wall Street",
      "category": "location-specific",
      "basePoints": 5,
      "locationRestriction": {"type": "neighborhood", "value": "wall-street"},
      "requirements": ["Must be on Wall Street", "Photo of climbing", "Be safe!"]
    },
    {
      "id": "location-012",
      "title": "Crown yourself in Crown Heights",
      "description": "Your team must parade around one designated 'ruler' around one block in Crown Heights and declare them the new ruler",
      "category": "location-specific",
      "basePoints": 10,
      "locationRestriction": {"type": "neighborhood", "value": "crown-heights"},
      "requirements": ["Must be in Crown Heights", "Parade around full block", "Video required", "Share with group"]
    },
    {
      "id": "location-013",
      "title": "Make someone flush in Flushing",
      "description": "Compliment 5 store workers in Flushing to make them blush",
      "category": "location-specific",
      "basePoints": 8,
      "locationRestriction": {"type": "neighborhood", "value": "flushing"},
      "requirements": ["Must be in Flushing", "5 different store workers", "Video evidence"]
    },
    {
      "id": "location-014",
      "title": "Hunt in Hunters Point",
      "description": "Capture a picture of wildlife (any animal) in Hunters Point",
      "category": "location-specific",
      "basePoints": 8,
      "locationRestriction": {"type": "neighborhood", "value": "hunters-point"},
      "requirements": ["Must be in Hunters Point", "Must be real wildlife", "Clear photo"]
    },
    {
      "id": "location-015",
      "title": "Bush hunting in Bushwick or Flatbush",
      "description": "Find THE bush of Bushwick or Flatbush. Make a 1 minute case for why your bush is THE bush",
      "category": "location-specific",
      "basePoints": 9,
      "locationRestriction": {"type": "neighborhood", "value": "bushwick-flatbush"},
      "requirements": ["Must be in Bushwick or Flatbush", "1 minute video case", "Show the bush"]
    },
    {
      "id": "location-016",
      "title": "Develop your green thumb in Greenpoint",
      "description": "Correctly identify 10 plant species found in Greenpoint",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "greenpoint"},
      "requirements": ["Must be in Greenpoint", "10 different plant species", "Photo of each with identification"]
    },
    {
      "id": "location-017",
      "title": "Eat a dumpling at a Chinatown",
      "description": "Eat a dumpling at any Chinatown in NYC",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "chinatown"},
      "requirements": ["Must be in a Chinatown", "Photo of dumpling", "Photo of restaurant"]
    },
    {
      "id": "location-018",
      "title": "Forrest in Forest Hills",
      "description": "Act like Forrest Gump in Forest Park",
      "category": "location-specific",
      "basePoints": 7,
      "locationRestriction": {"type": "neighborhood", "value": "forest-hills"},
      "requirements": ["Must be in Forest Park", "Video of Forrest Gump impression", "Stay in character"]
    },
    {
      "id": "location-019",
      "title": "Bushwick Collective street art",
      "description": "Visit the Bushwick Collective street art area and take photos",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "bushwick"},
      "requirements": ["Must be at Bushwick Collective", "Photo of street art"]
    },
    {
      "id": "location-020",
      "title": "Take a picture with your favorite statue in the 14th/8th St station",
      "description": "Find and photograph your favorite statue in the 14th Street/8th Avenue station",
      "category": "location-specific",
      "basePoints": 4,
      "locationRestriction": {"type": "neighborhood", "value": "14th-8th-station"},
      "requirements": ["Must be in 14th/8th St station", "Photo with statue"]
    },
    {
      "id": "location-021",
      "title": "Visit the Mosaic House in Williamsburg",
      "description": "Visit the famous Mosaic House in Williamsburg",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "williamsburg"},
      "requirements": ["Must be at the Mosaic House", "Photo of the house"]
    },
    {
      "id": "location-022",
      "title": "Visit the Socrates Sculpture Park",
      "description": "Visit Socrates Sculpture Park in Queens",
      "category": "location-specific",
      "basePoints": 6,
      "locationRestriction": {"type": "neighborhood", "value": "astoria"},
      "requirements": ["Must be at Socrates Sculpture Park", "Photo of sculptures"]
    }
  ]
};
