// Embedded hard challenges data for file:// protocol compatibility
window.CHALLENGES_HARD = {
  "challenges": [
    {
      "id": "hard-001",
      "title": "Give a fake museum explanation",
      "description": "Go to a museum/gallery and give a fake explanation about a piece to bystanders",
      "category": "hard",
      "basePoints": 15,
      "requirements": []
    },
    {
      "id": "hard-002",
      "title": "Find and interview at a food festival/community event",
      "description": "Find a food festival/community event and interview a staff member about it",
      "category": "hard",
      "basePoints": 12,
      "requirements": []
    },
    {
      "id": "hard-003",
      "title": "Make street art using chalk",
      "description": "Create street art using chalk",
      "category": "hard",
      "basePoints": 8,
      "requirements": []
    },
    {
      "id": "hard-004",
      "title": "Make your case to become the new secretary general",
      "description": "Make your case to become the new UN secretary general for at least 2 minutes with UN building in sight",
      "category": "hard",
      "basePoints": 10,
      "requirements": ["UN building must be visible"]
    },
    {
      "id": "hard-005",
      "title": "Tour an apartment that's up for rent soon",
      "description": "Tour an apartment that will be up for rent soon",
      "category": "hard",
      "basePoints": 15,
      "requirements": []
    },
    {
      "id": "hard-006",
      "title": "Go to an airport and pretend you're an airplane",
      "description": "Go to an airport and pretend you're an airplane for 3 minutes",
      "category": "hard",
      "basePoints": 20,
      "requirements": []
    },
    {
      "id": "hard-007",
      "title": "Make money peddling",
      "description": "Make money by peddling/selling something. Most money made gets bonus points",
      "category": "hard",
      "basePoints": 13,
      "bonusPoints": {"criteria": "Most money made", "points": 7},
      "requirements": []
    },
    {
      "id": "hard-008",
      "title": "Make up a secret handshake with a random New Yorker",
      "description": "Create and perform a secret handshake with a stranger",
      "category": "hard",
      "basePoints": 10,
      "requirements": []
    },
    {
      "id": "hard-009",
      "title": "Make a sandcastle at a beach",
      "description": "Build a sandcastle at a NYC beach",
      "category": "hard",
      "basePoints": 12,
      "requirements": []
    },
    {
      "id": "hard-010",
      "title": "Take a selfie with 4 strangers in 4 different NY major sports teams",
      "description": "Get selfies with 4 different people wearing 4 different NY major sports teams gear",
      "category": "hard",
      "basePoints": 12,
      "requirements": []
    },
    {
      "id": "hard-011",
      "title": "Act like a farmer at the farmers market",
      "description": "Go to a farmers market and act like a farmer",
      "category": "hard",
      "basePoints": 8,
      "requirements": ["Must stay in character"]
    },
    {
      "id": "hard-012",
      "title": "Stand on 5 distinct islands",
      "description": "Visit and stand on 5 different islands in NYC",
      "category": "hard",
      "basePoints": 20,
      "requirements": []
    },
    {
      "id": "hard-013",
      "title": "Stand at both ends of the same subway line",
      "description": "Visit both terminal stations of the same subway line at some point in the day",
      "category": "hard",
      "basePoints": 10,
      "requirements": []
    }
  ]
};
